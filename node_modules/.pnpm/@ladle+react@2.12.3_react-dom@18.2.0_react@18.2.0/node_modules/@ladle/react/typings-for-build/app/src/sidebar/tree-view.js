...
import { getEndId, getFirstChildId, getNextId, getParentId, getPrevId, toggleIsExpanded, } from "./utils";
const TreeView = ({ stories, story, updateStory, searchActive, searchRef, setTreeRootRef, }) => {
    ...
    return (_jsx("ul", { role: "tree", style: { marginInlineStart: "-6px" }, ref: (el) => setTreeRootRef(el), children: _jsx(NavigationSection, { tree: tree, fullTree: tree, story: story, updateStory: updateStory, onItemClick: (item) => setTree(toggleIsExpanded(tree, item)), selectedItemId: selectedItemId, onKeyDownFn: onKeyDownFn, treeItemRefs: treeItemRefs }) }));
};
const NavigationSection = ({ tree, fullTree, story, updateStory, onItemClick, onKeyDownFn, selectedItemId, treeItemRefs, }) => {
    return (_jsx(React.Fragment, { children: tree.map((treeProps) => {
            return (_jsxs("li", { onKeyDown: (e) => onKeyDownFn(e, treeProps), "aria-expanded": treeProps.isExpanded, title: treeProps.name, tabIndex: treeProps.id === selectedItemId && !treeProps.isLinkable ? 0 : -1, ref: treeProps.isLinkable
                    ? undefined
