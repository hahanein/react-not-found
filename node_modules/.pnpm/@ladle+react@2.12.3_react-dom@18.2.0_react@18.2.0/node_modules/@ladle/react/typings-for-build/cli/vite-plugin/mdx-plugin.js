...
function mdxPlugin(opts) {
    ...
    return {
        ...
        configResolved: ({ plugins }) => {
            ...
            reactPluginSwcTransform = plugins.find((p) => p.name === "vite:react-swc" && typeof p.transform === "function")?.transform;
        },
        async transform(value, path) {
            ...
            if (path.endsWith(".mdx")) {
                ...
                if (!reactPluginSwcTransform && !reactPluginTransform) {
                    throw new Error(`You need to install @vitejs/plugin-react or @vitejs/plugin-react-swc so ${filename} can be compiled.`);
                }
                if (reactPluginSwcTransform) {
                    // swc plugin can compile JSX, no need for esbuild
                    return await reactPluginSwcTransform(code, filepath.replace(".mdx", ".jsx"));
                }
                return await reactPluginTransform((await transformWithEsbuild(code, filename)).code, filename);
            }
        },
    };
}
